"""V10 Noise Injection Experiment: Why Random Wins?

Tests the hypothesis that diverse incorrect data acts as regularization,
explaining why Random (6.8% correct) beats QD (60% correct) at k=500.
...
Plus existing baselines:
  - QD-500 (60% correct, 71 cells): 14.5±3.0% HumanEval
  - Random-500 (6.8% correct, 53 cells): 47.7±11.5% HumanEval

Usage:
  python v10_noise_injection.py --gpu 1 --config random_incorrect
  python v10_noise_injection.py --gpu 2 --config qd_correct_random_incorrect
  python v10_noise_injection.py --gpu 3 --config stratified_random
"""
import os
...
OUT_DIR = RESULTS_DIR / "noise_injection"
...
def make_random_incorrect(k=500, seed=42):
    """500 random incorrect solutions (0% correct)."""
    rng = random.Random(seed)
    return rng.sample(incorrect_pool, min(k, len(incorrect_pool)))

def make_qd_correct_random_incorrect(k=500, seed=42):
    """300 QD-selected correct + 200 random incorrect (60% correct)."""
    # QD selection from correct pool
    cells_dict = {}
    ...
    qd_correct = [cells_dict[key][0] for key in cells_dict]  # 35 cells
    # Fill to 300 with remaining correct
...
def make_stratified_random(k=500, seed=42):
    """1 random per cell (71 cells) + random fill."""
    rng = random.Random(seed)
...
def train_and_eval_he(selected, config_name, lora_seed):
    """Train LoRA with specific seed and evaluate HumanEval."""
...
for seed in ALL_SEEDS:
    ...
    # Build training data (selection seed varies per config)
