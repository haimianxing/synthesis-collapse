"""
Per-Round Downstream with Real Data Selection
Uses iterative v2 archives as SELECTORS for real MBPP/GSM8K data.
Key insight: instead of fine-tuning on API-generated data (0% pass@1),
use archive cell structure to select diverse real training data.

Shows: Greedy collapse → downstream stagnation; QD expansion → downstream improvement.
This is the critical missing link in the collapse narrative.
"""
...
print(f"=== Per-Round Real Downstream: {DOMAIN.upper()} (GPU {GPU_ID}) ===", flush=True)

# ============ Domain-specific setup ============
if DOMAIN == "code":
    ITER_DIR = Path("/mnt/data2/zcz/neurIps-emnlp/neurips/results/code_iterative_v2")
    ROUNDS_TO_TEST = {"greedy": [0,1,2,3,5,7], "qd": [0,1,2,3]}
    ...
    # Build MBPP pool with descriptors
    pool = []
    for ex in mbpp:
        code = ex.get('code', '')
    ...
    print(f"MBPP pool: {len(pool)} samples", flush=True)
    ...
    ROUNDS_TO_TEST = {"greedy": [0,1,2,3,5,7], "qd": [0,1,2,3,5]}
    ...
    # Use 200 test samples (matching original experiment design, avoids 7473-sample bottleneck)
    random.seed(42)
    gsm8k_test = random.sample(list(gsm8k_test_full), min(200, len(gsm8k_test_full)))
    print(f"GSM8K pool (test): {len(gsm8k_pool)}, eval subset: {len(gsm8k_test)}/7473", flush=True)
    ...
    print(f"GSM8K pool: {len(pool)} samples", flush=True)
...
def load_archive_cells(archive_path):
    """Load archive and extract occupied cells."""
...
def select_real_data_for_cells(cells):
    """Select highest-quality real data sample for each occupied cell."""
    cell_to_best = {}
    for item in pool:
...
# ============ Main: Per-round evaluation ============
all_results = {}

for strategy in ["greedy", "qd"]:
    print(f"\n{'='*50}\n{strategy.upper()} Per-Round Evaluation ({DOMAIN})\n{'='*50}", flush=True)

    for rnd in ROUNDS_TO_TEST[strategy]:
        ...
        for seed in [42, 123, 271][:N_SEEDS]:
            result = finetune_and_eval(real_data, f"{strategy}_r{rnd}", seed)
            result['seed'] = seed
        ...
        # Save intermediate
        with open(RESULTS_DIR / f"{DOMAIN}_per_round_real_results.json", "w") as f:
            json.dump(all_results, f, indent=2, default=str)

# ============ Summary ============
print(f"\n{'='*60}", flush=True)
print(f"PER-ROUND REAL DOWNSTREAM RESULTS ({DOMAIN.upper()})", flush=True)
...
for strategy in ["greedy", "qd"]:
    ...
    for rnd in ROUNDS_TO_TEST[strategy]:
        ...
        if key in all_results:
            r = all_results[key]
            print(f"    R{rnd}: {metric_name}={r[metric_name]}±{r['std']}, cells={r['n_cells']}, real={r['n_real_selected']}", flush=True)
print(f"{'='*60}", flush=True)

with open(RESULTS_DIR / f"{DOMAIN}_per_round_real_results.json", "w") as f:
    json.dump(all_results, f, indent=2, default=str)
print(f"Results saved: {RESULTS_DIR}", flush=True)
