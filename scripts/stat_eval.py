"""
Multi-seed statistical significance test for downstream evaluation.
Run the same evaluation 3 times with different generation seeds,
compute mean ± std for each metric across runs.
Uses GPU 2-4 for parallel runs.
"""
import sys, os
sys.path.insert(0, "/mnt/data2/zcz/neurIps-emnlp/neurips/scripts")

# Import evaluation functions from the existing eval script
...
N_PROMPTS = 25  # Use 25 prompts for faster evaluation


def evaluate_once(model_name, gpu_id, seed):
    """One evaluation run with specific seed"""
    ...
    # Shuffle prompts with this seed
...
if __name__ == "__main__":
    ...
    print(f"Evaluating {model_name} on GPU {gpu_id} with seed={seed}")
    result = evaluate_once(model_name, gpu_id, seed)
    result["model"] = model_name
    result["seed"] = seed

    out_path = RESULTS_DIR / f"stat_{model_name}_seed{seed}.json"
